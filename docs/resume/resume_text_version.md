...
- 问题：地图DeepSearch场景缺乏高质量Agent训练数据，人工标注单条成本超过50元且效率低，而通用Agent数据集（ToolBench、API-Bank）与地图场景偏差大，直接使用会导致领域迁移效果差
- 方案：基于OpenManus框架搭建ReAct Agent造数Pipeline，分三个阶段：

  **第一阶段：多样化Query生成。** 使用DeepSeek-V3作为Query Generator，通过few-shot prompt生成地图场景Query。覆盖8大场景：地点搜索（"附近的星巴克"）、沿途搜索（"回家路上找个加油站"）、附近搜索（"公司周边有什么吃的"）、多点路线（"先去超市再去学校怎么走"）、家/公司上下文（"送我回家"）、品牌词搜索（"最近的瑞幸"）、模糊意图（"想吃火锅"）、跨城市查询（"北京到上海沿途景点"）。每个场景生成1w+条Query，按城市线级（一线/新一线/二线）和时间段（早高峰/午间/晚间）分层采样，总计生成10w+条原始Query。

  **第二阶段：ReAct轨迹采集。** 将生成的Query输入基于OpenManus搭建的ReAct Agent，Agent可调度12个地图工具（geocode、poi_search、route_plan、nearby_search等）和Web Search工具。Agent按"思考→工具调用→观察→继续推理→回答"循环执行，每一步的思考过程、工具选择、参数填充、工具返回结果均被完整记录。Web Search采用4路并行检索真实POI信息，为Agent提供背景知识。每条Query平均执行3.2步，生成完整的ReAct轨迹（thought→action→observation序列）。

  **第三阶段：质量过滤与分级。** 对采集的轨迹进行三阶段筛选：(1) 格式校验：检查工具调用JSON格式、参数类型、必填字段完整性，过滤格式错误约12%；(2) 执行验证：回放工具调用验证返回结果是否真实存在（POI是否存在、路线是否可达），过滤不可执行轨迹约8%；(3) 质量打分：使用Qwen2.5-72B对最终回答质量打分（0-1分，评估准确性、完整性、可读性），保留得分>0.7的轨迹。最终从10w+条原始轨迹中筛选出8w+条高质量ReAct轨迹。

  **数据分级：** 质量得分>0.9的3w+条作为SFT数据（用于监督微调）；其余5w+条通过偏好对构造（从同一Query的多条轨迹中选取最优和次优构成chosen/rejected对）作为GRPO偏好数据。

- 结果：构建8w+条ReAct轨迹数据（SFT数据3w+条，GRPO偏好数据5w+对），数据覆盖8大场景200+城市，轨迹平均步数3.2步，工具调用成功率89.7%。相比人工标注，造数效率提升约50倍，单条成本从50元降至1元以下。
